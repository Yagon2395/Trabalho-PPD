...
__global__
void d_main(int dim, int* d_vetcores, int* w, int* u, int vzin, int num_blocos){
  int aux = blockDim.x * blockIdx.x + threadIdx.x;
  int i;
  for(i = aux; i < dim; i += blockDim.x * num_blocos){
    if(vzin == i){
      for(int j =0; j < dim; j++){
        if(w[j +i*dim] == 1){
...
int main(){
  ...
  while (ha_elementos_a_colorir(vetcores, dim)){
    ...
    while(possuiAresta(w,dim*dim)){
      ...
      tStart = clock();
      d_main<<<num_blocos,num_threads_por_bloco>>>(dim, d_vetcores, d_w, d_u, vzin, num_blocos);
